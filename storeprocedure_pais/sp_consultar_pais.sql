...
Propósito: Implementación oficial y optimizada del Stored Procedure para el Inciso e).
           ...
           - Soporte desacoplado para NOCs históricos/especiales con id_entidad IS NULL
             (ej. ROC, EOR, ROT, AIN, COR).
           ...
           - Control robusto de ambigüedad, validación de dominio insensible a acentos
             en @temporada y protección contra caracteres comodín en LIKE.
...
CREATE OR ALTER PROCEDURE olympics.sp_consultar_pais
    @pais_o_noc           NVARCHAR(150),       -- Código NOC (ej. 'GUA', 'FRA', 'ROC'), Código ISO (ej. 'GTM') o Nombre del País (ej. 'Guatemala', 'Francia')
...
BEGIN
    ...
    BEGIN
        RAISERROR(N'ERROR: El parámetro @pais_o_noc no puede estar vacío o nulo. Proporcione un código NOC (ej. ''GUA'', ''ROC''), código ISO (ej. ''GTM'') o nombre de país (ej. ''Guatemala'').', 16, 1);
        RETURN;
    END;

    -- 1.2 Validación insensible a mayúsculas y acentos del dominio de @temporada (Requisito 6 de Auditoría)
    IF @temporada IS NOT NULL
    BEGIN
        SET @temporada = TRIM(@temporada);
        IF @temporada COLLATE Latin1_General_CI_AI NOT IN (N'Summer', N'Winter', N'Summer Youth', N'Winter Youth', N'Intercalated Games')
        BEGIN
        ...
        -- Normalizar a título canónico para comparaciones determinísticas
        SET @temporada = CASE 
            WHEN @temporada COLLATE Latin1_General_CI_AI = N'Summer' THEN N'Summer'
            WHEN @temporada COLLATE Latin1_General_CI_AI = N'Winter' THEN N'Winter'
            WHEN @temporada COLLATE Latin1_General_CI_AI = N'Summer Youth' THEN N'Summer Youth'
            WHEN @temporada COLLATE Latin1_General_CI_AI = N'Winter Youth' THEN N'Winter Youth'
            WHEN @temporada COLLATE Latin1_General_CI_AI = N'Intercalated Games' THEN N'Intercalated Games'
            ELSE @temporada
        END;
    END;
    ...
    -- ====================================================================================
    -- PASO 2: RESOLUCIÓN JERÁRQUICA DESACOPLADA (@id_entidad vs @id_noc)
    -- ====================================================================================
    -- Resuelve tanto entidades geográficas nacionales como delegaciones especiales/independientes
    -- con id_entidad IS NULL (ej. ROC, EOR, ROT, AIN, COR).
    -- ====================================================================================

    DECLARE @id_entidad INT = NULL;
    DECLARE @id_noc INT = NULL;
    DECLARE @codigo_noc_exacto NVARCHAR(10) = NULL;
    DECLARE @nombre_noc_exacto NVARCHAR(150) = NULL;

    -- 2.1 Búsqueda exacta por código NOC
    SELECT TOP 1 
        @id_entidad = n.id_entidad,
        @id_noc = n.id_noc,
        @codigo_noc_exacto = n.codigo_noc,
        @nombre_noc_exacto = n.nombre_noc
    FROM olympics.NOC n
    WHERE n.codigo_noc = UPPER(@param_limpio);

    -- 2.2 Búsqueda exacta por código ISO de país
    IF @id_entidad IS NULL AND @id_noc IS NULL
    BEGIN
    ...
    -- 2.3 Búsqueda exacta por nombre de entidad geográfica o nombre de NOC
    IF @id_entidad IS NULL AND @id_noc IS NULL
    BEGIN
        SELECT TOP 1 @id_entidad = eg.id_entidad
        FROM olympics.ENTIDAD_GEOGRAFICA eg
        ...
        BEGIN
            SELECT TOP 1 
                ...
                @nombre_noc_exacto = n.nombre_noc
            FROM olympics.NOC n
            WHERE n.nombre_noc = @param_limpio COLLATE Latin1_General_CI_AI;
        END;
    END;
    ...
    BEGIN
        ...
        -- Candidatos con entidad geográfica
        INSERT INTO @candidatos (id_entidad, id_noc, nombre, codigo_iso, codigos_noc)
        SELECT DISTINCT
            eg.id_entidad,
            NULL AS id_noc,
            eg.nombre,
        ...
        -- Candidatos especiales sin entidad geográfica (ROC, EOR, ROT, AIN, COR)
        INSERT INTO @candidatos (id_entidad, id_noc, nombre, codigo_iso, codigos_noc)
        SELECT 
            NULL AS id_entidad,
            n.id_noc,
            ISNULL(n.nombre_noc, n.codigo_noc) AS nombre,
            N'N/A' AS codigo_iso,
            n.codigo_noc AS codigos_noc
        FROM olympics.NOC n
        WHERE n.id_entidad IS NULL
          AND (n.codigo_noc LIKE N'%' + @param_escapado + N'%' COLLATE Latin1_General_CI_AI
               OR n.nombre_noc LIKE N'%' + @param_escapado + N'%' COLLATE Latin1_General_CI_AI);
        ...
        BEGIN
            SELECT 
                ...
                @id_noc = id_noc,
                @codigo_noc_exacto = codigos_noc,
                @nombre_noc_exacto = nombre
            FROM @candidatos;
        END;
        ELSE
        BEGIN
            SELECT 
                ...
                CONCAT(N'Se encontraron ', @total_candidatos, N' entidades/NOCs que coinciden con "', @pais_o_noc, N'". Por favor refine su búsqueda utilizando el código NOC exacto o el nombre completo.') AS mensaje;

            SELECT 
                id_entidad,
                id_noc,
                nombre AS pais_o_delegacion,
                ISNULL(codigo_iso, N'N/A') AS codigo_iso,
    ...
    -- ====================================================================================
    -- PASO 2.5: POBLAR TABLA DE NOCS PERTINENTES AL FILTRO
    -- ====================================================================================
    -- Si el país tiene id_entidad, abarca todos sus códigos NOC históricos (ej. Alemania -> GER, FRG, GDR, SAA).
    -- Si es un NOC especial sin entidad (ej. ROC), abarca únicamente dicho id_noc.
    -- ====================================================================================
    DECLARE @nocs_filtrados TABLE (id_noc INT PRIMARY KEY);

    IF @id_entidad IS NOT NULL
    BEGIN
        INSERT INTO @nocs_filtrados (id_noc)
        SELECT n.id_noc FROM olympics.NOC n WHERE n.id_entidad = @id_entidad;
    END
    ELSE
    BEGIN
        INSERT INTO @nocs_filtrados (id_noc)
    ...
    BEGIN
        SELECT 
            ...
            ), N'Sin NOC') AS codigos_noc_historicos,
    ...
    BEGIN
        SELECT 
            CAST(NULL AS INT) AS id_entidad,
            ISNULL(@nombre_noc_exacto, @codigo_noc_exacto) AS pais,
            N'N/A' AS codigo_iso_pais,
            @codigo_noc_exacto AS codigos_noc_historicos,
            CAST(NULL AS BIGINT) AS poblacion_mas_reciente,
            CAST(NULL AS SMALLINT) AS anio_censo_poblacion;
    END;
    ...
    BEGIN
        SELECT 
            N'NO' AS ha_sido_sede,
            0 AS total_ediciones_como_sede,
            N'Ninguna' AS ciudades_sede,
            N'Este equipo o delegación independiente nunca ha sido sede de Juegos Olímpicos' AS resumen_ediciones_como_sede;
    END;
    ...
    BEGIN
        SELECT 
            CAST(NULL AS INT) AS id_edicion,
            CAST(NULL AS SMALLINT) AS anio,
            CAST(NULL AS NVARCHAR(50)) AS temporada,
            CAST(NULL AS NVARCHAR(150)) AS ciudad_sede
        WHERE 1 = 0;
    END;
    ...
    -- 1. Medallas oficiales del país: La clave lógica oficial es (id_edicion + id_evento + id_noc + medalla).
    --    Evita inflar oros en deportes de equipo (ej. 1 oro oficial para fútbol de Argentina 2008 en vez de 18).
    -- 2. Preseas físicas individuales: Conteo de todas las medallas entregadas a deportistas.
    -- ====================================================================================
    SELECT 
        (
            SELECT COUNT(DISTINCT p1.id_participacion)
            FROM olympics.PARTICIPACION p1
            JOIN @nocs_filtrados nf1 ON nf1.id_noc = p1.id_noc
            ...
            WHERE (@anio IS NULL OR eo1.anio = @anio)
              AND (@temporada IS NULL OR eo1.temporada COLLATE Latin1_General_CI_AI = @temporada)
        ...
        (
            ...
            JOIN @nocs_filtrados nf2 ON nf2.id_noc = p2.id_noc
            ...
            WHERE (@anio IS NULL OR eo2.anio = @anio)
              AND (@temporada IS NULL OR eo2.temporada COLLATE Latin1_General_CI_AI = @temporada)
        ...
        (
            ...
            JOIN @nocs_filtrados nf3 ON nf3.id_noc = p3.id_noc
            ...
            WHERE p3.medalla IS NOT NULL
              AND (@anio IS NULL OR eo3.anio = @anio)
              AND (@temporada IS NULL OR eo3.temporada COLLATE Latin1_General_CI_AI = @temporada)
    ...
    FROM (
        ...
        JOIN @nocs_filtrados nf ON nf.id_noc = p.id_noc
    ...
    SELECT TOP (@top_participaciones)
        ...
        a.nombre AS atleta,
        n.codigo_noc,
        ISNULL(n.nombre_noc, n.codigo_noc) AS nombre_noc,
        ISNULL(eg.nombre, N'N/A') AS pais_representado,
        p.equipo,
        p.posicion,
    ...
    ORDER BY 
        CASE 
            WHEN p.medalla = N'Gold' THEN 1
            WHEN p.medalla = N'Silver' THEN 2
            WHEN p.medalla = N'Bronze' THEN 3
            ELSE 4
        END ASC,
        eo.anio DESC,
        dep.nombre ASC,
